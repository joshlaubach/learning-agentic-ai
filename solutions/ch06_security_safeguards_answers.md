...
it's cheap and it helps at the margin — but presenting it as *the* fix, rather than one layer
among several, is the actual red flag an interviewer is likely listening for here.
...
2. **Restrict `to` to a fixed allowlist (e.g., only the user's own saved contacts).** An
   attacker can no longer exfiltrate to an address of their choosing — the most damaging
   single restriction, since it closes off exfiltration as a goal entirely, even if the
   agent can still be tricked into *sending* something.
