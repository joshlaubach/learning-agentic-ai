...
- If token counts per request look normal but the number of *distinct* calls per user
  session went up, suspect something upstream retrying or fanning out more than before, the
  same root cause at a different layer (session-level duplication instead of per-request
  duplication).
...
- If `queue_time_ms`'s share of total latency jumped (this chapter's break-it #3: input/
  output tokens stay normal, `queue_depth` spikes), that's a capacity/backpressure problem:
  too many concurrent requests for available capacity, most likely a burst (a batch job, a
  retry storm, a real traffic spike) rather than anything about any individual request. The
  fix is capacity and load-shedding (reject past a depth threshold so latency degrades
  predictably instead of catastrophically), not touching prompts or models.
...
- If input tokens per request grew a lot (break-it #2's signature) while stage *shares*
  ...
  there's more to process," which is a context-growth problem wearing a latency costume. The
  fix is a context window policy, not more GPUs.
...
- Is this a consistent, high-volume, stable requirement, or is the desired tone still
  ...
  retraining every time the target shifts. It's a much better fit once the target has
  stabilized.
...
- Whose time and infrastructure does a fine-tune consume, and is that justified relative
  ...
  to, and a new evaluation pass all cost more than a prompt change. It's worth naming
  explicitly as a cost/benefit question rather than assuming it away.
